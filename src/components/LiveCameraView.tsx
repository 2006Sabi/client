...
import { Eye, Wifi, WifiOff, RefreshCw, Settings, Trash2, Plus, X, Grid3X3, List } from 'lucide-react';
...
export const LiveCameraView = () => {
  ...
  const addCustomCamera = () => {
    if (customCameraId.trim() && !selectedCameras.includes(customCameraId.trim())) {
  ...
  const getStreamUrl = (camera: { _id: string; httpUrl?: string }) => {
    // Use the public httpUrl if available, else fallback to localIp construction
    ...
    return localIp ? `http://${localIp}:8080/stream/${camera._id}` : '';
  };

  const cameras = camerasData?.data || [];
  const displayCameras = selectedCameras.length > 0 
    ? selectedCameras.map(id => cameras.find(cam => cam._id === id) || { 
        _id: id, 
    ...
    return (
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-foreground mb-2">Live Camera Streaming</h2>
            <p className="text-muted-foreground">Monitor live streams from your security cameras</p>
          </div>
        </div>
        <CameraGridSkeleton />
      </div>
    ...
    <div className="space-y-8">
      ...
      <div className={viewMode === 'grid' 
        ...
        {displayCameras.map((camera) => {
          ...
          return (
            <Card key={camera._id} className="modern-card overflow-hidden group hover:shadow-lg transition-all duration-300">
                ...
                <CardTitle className="flex items-center justify-between text-lg">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-semibold truncate">{camera.name}</div>
                      <div className="text-sm text-muted-foreground font-normal truncate">{camera.location}</div>
                    </div>
                  </div>
                  <Badge 
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="text-muted-foreground">
                    {camera.created_at ? `Added: ${new Date(camera.created_at).toLocaleDateString()}` : 'Custom Camera'}
                  </div>
